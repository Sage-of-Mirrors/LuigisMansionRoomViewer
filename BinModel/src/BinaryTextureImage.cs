...
namespace BinModel.src
{
    ...
    public class BinaryTextureImage
    {
        ...
        public BinaryTextureImage(BinaryTextureImage original)
        {
            Format = original.Format;
            Width = original.Width;
            Height = original.Height;
            m_rgbaImageData = original.GetData();
        }
        ...
        /// <summary>
        /// Loads a texture from the .bin model format used in Luigi's Mansion.
        /// </summary>
        /// <param name="stream">Stream to read the data from</param>
        /// <param name="startOffset">Offset of the start of the TexBlock</param>
        public void LoadBinTex(EndianBinaryReader stream, long startOffset)
        {
            Width = stream.ReadUInt16();
            Height = stream.ReadUInt16();
            Format = (TextureFormats)stream.ReadByte();
            ...
            long curOffset = stream.BaseStream.Position;

            stream.BaseStream.Seek(startOffset + dataOffset, SeekOrigin.Begin);
            m_rgbaImageData = DecodeData(stream, Width, Height, Format, new Palette(), PaletteFormats.IA8);
            stream.BaseStream.Seek(curOffset, SeekOrigin.Begin);
        }

        public void SetTexSettingsFromBin(EndianBinaryReader reader)
        {
